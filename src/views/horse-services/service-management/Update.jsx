...
function Update({ dispatch, pageReducer }) {
  ...
  const onSubmit = (id, values) => {
    axios
      .put(`/api/admin-accounts/${id}`, values)
      .then((response) => {
        console.log(response);
        addToast("Cập nhật thành công", {
          appearance: "success",
          autoDismiss: true,
        });
        dispatch(goToList());
  ...
  return (
    <>
      <div className="intro-y flex items-center mt-8">
        <h2 className="text-lg font-medium mr-auto">Cập nhật tài khoản</h2>
      </div>
            ...
            <div className="text-right mt-5">
              ...
              <button
                ...
                onClick={handleSubmit(function (values) {
                  onSubmit(pageReducer?.record?._id, values);
                })}
              >
                Cập nhật
              </button>
